...
public class ArenaManager {
    ...
    public Optional<Arena> findBestJoinable() {
        return arenas.values().stream()
                .filter(Arena::canJoin)
                .filter(Arena::isReady)
                .filter(a -> a.getPlayerCount() < a.getMaxPerTeam() * 2)
    ...
    public void save() {
        ...
        if (lobbySpawn != null) {
            LocationUtil.save(cfg, "lobby-spawn", lobbySpawn);
        }
        for (Arena arena : arenas.values()) {
            ...
            for (Location loc : arena.getChestLocations()) {
                chestList.add(LocationUtil.serializeSimple(loc));
            }
