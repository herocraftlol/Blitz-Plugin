...
public class BlitzCommand implements CommandExecutor, TabCompleter {
    ...
    private static final List<String> PLAYER_SUBS = List.of("join", "leave", "gui", "stats");
    private static final List<String> ADMIN_SUBS = List.of("wand", "create", "delete", "list", "setlobby",
            "setspawn", "setregion", "setgoal", "setlimit", "setteamsize", "addchest", "forcestart", "reload",
    ...
    private void handleJoin(CommandSender sender, String[] args) {
        ...
        if (args.length < 2) {
            player.sendMessage(ChatColor.RED + "Usage: /blitz join <arène>");
            return;
        }
    ...
    private void handleCreate(CommandSender sender, String[] args) {
        ...
        if (arena == null) {
            sender.sendMessage(ChatColor.RED + "Une arène avec ce nom existe déjà.");
            return;
        }
        sender.sendMessage(ChatColor.GREEN + "Arène " + ChatColor.GOLD + args[1] + ChatColor.GREEN + " créée. "
                + "Configurez-la avec /blitz setspawn, setgoal, setregion, addchest...");
    }
    ...
    private void handleList(CommandSender sender) {
        ...
        for (Arena arena : plugin.getArenaManager().getAll()) {
            sender.sendMessage(ChatColor.YELLOW + "- " + arena.getName() + ChatColor.GRAY
                    + " | état: " + arena.getState()
                    + " | joueurs: " + arena.getPlayerCount() + "/" + (arena.getMaxPerTeam() * 2)
                    + " | prête: " + (arena.isReady() ? "oui" : "non"));
        }
    }
    ...
    private void handleSetLobby(CommandSender sender) {
        ...
        sender.sendMessage(ChatColor.GREEN + "Spawn du lobby défini.");
    }
    ...
    private void handleSetRegion(CommandSender sender, String[] args) {
        ...
        if (sel == null || sel[0] == null || sel[1] == null) {
            sender.sendMessage(ChatColor.RED + "Sélectionnez d'abord deux points avec /blitz wand.");
            return;
        }
    ...
    private void handleSetGoal(CommandSender sender, String[] args) {
        if (!checkAdmin(sender) || !(sender instanceof Player player)) return;
        if (args.length < 3) { sender.sendMessage(ChatColor.RED + "Usage: /blitz setgoal <red|blue> <arène>"); return; }
        Arena arena = plugin.getArenaManager().get(args[2]);
        if (arena == null) { sender.sendMessage(ChatColor.RED + "Arène introuvable."); return; }
        Location[] sel = plugin.getSelections().get(player.getUniqueId());
        if (sel == null || sel[0] == null || sel[1] == null) {
        ...
        if (args[1].equalsIgnoreCase("red")) {
            ...
            sender.sendMessage(ChatColor.GREEN + "Trou rouge défini pour " + arena.getName() + ".");
        } else if (args[1].equalsIgnoreCase("blue")) {
            arena.setBlueGoal(region);
            sender.sendMessage(ChatColor.GREEN + "Trou bleu défini pour " + arena.getName() + ".");
        } else {
            sender.sendMessage(ChatColor.RED + "Équipe invalide (red/blue).");
            return;
        }
    ...
    private void handleSetTeamSize(CommandSender sender, String[] args) {
        ...
        try {
            ...
            if (max < 1 || max > 8) {
                sender.sendMessage(ChatColor.RED + "La taille d'équipe doit être comprise entre 1 et 8.");
                return;
            }
            arena.setMaxPerTeam(max);
            plugin.getArenaManager().save();
            sender.sendMessage(ChatColor.GREEN + "Taille d'équipe max définie à " + max + " (" + max + "v" + max
                    + ") pour " + arena.getName() + ".");
        } catch (NumberFormatException e) {
            sender.sendMessage(ChatColor.RED + "Nombre invalide.");
        }
    ...
    private void handleAddChest(CommandSender sender, String[] args) {
        ...
        if (target == null || !(target.getState() instanceof Chest)) {
            sender.sendMessage(ChatColor.RED + "Regardez un coffre pour l'ajouter (distance max 6 blocs).");
            return;
        }
    ...
    private void handleHologram(CommandSender sender, String[] args) {
        ...
        try {
            ...
            sender.sendMessage(ChatColor.RED + "Type invalide (wins/played/kd/kills).");
            return;
        }
    ...
    private void sendHelp(CommandSender sender) {
        ...
        sender.sendMessage(ChatColor.YELLOW + "/blitz leave" + ChatColor.GRAY + " - Quitter l'arène actuelle");
        ...
        if (sender.hasPermission("blitz.admin")) {
            ...
            sender.sendMessage(ChatColor.YELLOW + "/blitz setlobby" + ChatColor.GRAY + " - Définir le spawn du lobby");
            ...
            sender.sendMessage(ChatColor.YELLOW + "/blitz setgoal <red|blue> <arène>" + ChatColor.GRAY + " - Trou d'équipe");
            ...
            sender.sendMessage(ChatColor.YELLOW + "/blitz hologram <type>" + ChatColor.GRAY + " - Créer un classement hologramme");
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 2) {
            String sub = args[0].toLowerCase(Locale.ROOT);
            if (List.of("join", "delete", "setregion", "addchest", "forcestart", "setlimit", "setteamsize").contains(sub)) {
                return arenaNames(args[1]);
            }
            if (List.of("setspawn", "setgoal").contains(sub)) {
